...
if(opera == "collection"){
    ...
    .evaluate(() => {
        ...
        for(let i = 0;i < questionBodys.length;i += 2){
            let answerArr = []
            let answer
            let title = (questionBodys.item(i).querySelector(".vtbegenerated.inlineVtbegenerated") as any).innerText
            //正确答案
            let tmp = questionBodys.item(i + 1).querySelectorAll("tr")
            for(let j = 0;j < tmp.length;j++){
                if(tmp.item(j).querySelector(".correctAnswerFlag")) {
                    if(j == 1) {
                        answer = (tmp.item(j).querySelector(".reviewQuestionsAnswerDiv") as any).innerText
                    } else {
    ...
    (async () => {
                ...
                if(tArr.length != 0) {
                    ...
                    for(let val of (tArr[0] as any).answer){
                            ...
                            for(let i = 0;i < checkBoxRadioAnswer.length;i++){
                                ...
                                let text:string = (checkBoxRadioAnswer.item(i) as any).innerText
                                if(new RegExp(trim(text),"g").test(val)) {
                                    (checkBoxRadioAnswer.item(i).querySelector("label") as any).click()
                                }
                            }
                    }
                }
        ...
        .evaluate(() => {
            let result:string = (document.querySelector(".infoListWrapper").querySelector("tr:nth-child(7) td") as any).innerText.split(" ")[1];
            return result
        })
        .then((result:string) => {
            console.log(result)
